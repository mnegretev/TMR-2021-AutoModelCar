...
def laser(data):
    ...
    filter_data_frontal3=datos_frontal3[datos_frontal3 <=0.38]
    ...
    filter_data_t=datos_t[datos_t<2]
    ...
    filter_dc3=datos_c3[datos_c3<1]

    ## Cruce

    lidar_cruce=data.ranges[325:335]
    datos_cruce=np.array(lidar_cruce)
    filter_cruce=datos_cruce[datos_cruce<1]

    lidar_cruce2=data.ranges[265:275]
    datos_cruce2=np.array(lidar_cruce2)
    filter_cruce2=datos_cruce2[datos_cruce2<1]
    ...
    if estado==9:
        #print('estado 9')
        #print(len(filter_cruce2))
        if len(filter_cruce2)!=0:
            automatico=False
            dire.publish(15)
            sleep(0.1)
            automatico=True
