...
class LaneDetector:
    ...
    def process(self, image):
        ...
        #print(LineaL)
        ##        v2            v3          v4              v6          v7          v8
        gir = (0.05274671*float(LineaL[0][0]))+(0.03902426*float(LineaL[0][1])) + (0.03745892*float(LineaL[1][0]))+(0.04439124* float(LineaR[0][0])) + (-0.68796478* float(LineaR[0][1]))+ (0.15142310*float(LineaR[1][0]))
        #gir = (-0.03382*float(LineaL[0][0]))+(-0.10398*float(LineaL[0][1])) + (-0.03581*float(LineaL[1][0]))+(-0.05144* float(LineaR[0][0])) + (0.25599* float(LineaR[0][1]))+ (-0.07438*float(LineaR[1][0]))+(7.64578* float(Prom))+(-4.03470* float(prom2))
        #print(gir)
        gir = (-(gir))+90
        #print(gir)
        dire.publish(gir)
...
def callback(data):
    ...
    global Prom
    global prom2
    medida2= data.ranges[200:210]
    medida2 = np.array(medida2)
    medida2 = medida2[medida2<2.8]

    medida = data.ranges[0:30]
    medida = np.array(medida)
    medida = medida[medida<2.8]
    if len(medida)==0:
        Prom = 0
    else:
        Prom = np.average(medida)
    #print(Prom)
    if len(medida2)==0:

        prom2 = 0
    else:
        prom2 = np.average(medida2)
    #print(medida)
    #dire.publish(90)
    #vel.publish(0)
...
def start(args):
    ...
    vel = rospy.Publisher('peed', Int16,queue_size = 10)
    dire = rospy.Publisher('Angulo', Int16,queue_size = 10)
